...
#' @details
#' Uses \code{\link[evalITR]{consistcv.test}} for inference.
#' @examples
#' D = c(1,0,1,0,1,0,1,0)
...
consistcv.test <- function(D, tau, Y, ind, ngates = 5, nsim = 10000,
                      centered = TRUE) {
  evalITR::consistcv.test(T = D, tau = tau, Y = Y, ind = ind, ngates = ngates, nsim = nsim,
    centered = centered)
}

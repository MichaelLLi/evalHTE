...
#' @details
#' Uses \code{\link[evalITR]{hetcv.test}} for inference.
#' @examples
#' D = c(1,0,1,0,1,0,1,0)
...
#' @keywords evaluation
#' @export hetcv.test
hetcv.test <- function(D, tau, Y, ind, ngates = 5, centered = TRUE) {
  evalITR::hetcv.test(T = D, tau = tau, Y = Y, ind = ind, ngates = ngates,
    centered = centered)
}
